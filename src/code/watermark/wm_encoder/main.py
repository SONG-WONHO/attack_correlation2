...
from watermark.wm_encoder.models import SSIM
from watermark.wm_encoder.models import *
...
def watermark(config, log):
    ...
    if args.dataset == 'cifar10':
        ...
        trainloader = torch.utils.data.DataLoader(
            trainset, batch_size=args.batchsize, shuffle=False, num_workers=2,
            drop_last=True)
    ...
    if args.wm_train:
        for wm_idx, (wm_input, wm_cover_label) in enumerate(trigger_loader):
            ...
            if args.dataset == 'cifar10' and wm_idx == (int(args.wm_num[
                                                                0] / args.wm_batchsize) - 1):  # choose 1% of dataset as origin sample
                break
            elif args.dataset == 'mnist' and wm_idx == (
                    int(args.wm_num[1] / args.wm_batchsize) - 1):
                break
    ...
    schedulerH = ReduceLROnPlateau(optimizerH, mode='min', factor=0.2, patience=5,
                                   verbose=True)

    criterionD = nn.BCELoss()
    optimizerD = optim.Adam(Disnet.parameters(), lr=args.lr[0], betas=(0.5, 0.999))
    schedulerD = ReduceLROnPlateau(optimizerD, mode='min', factor=0.2, patience=8,
                                   verbose=True)

    criterionN = nn.CrossEntropyLoss()
    optimizerN = optim.SGD(Dnnet.parameters(), lr=args.lr[1], momentum=0.9,
                           weight_decay=5e-4)
    ...
    def train(epoch):
        ...
        for batch_idx, (input, label) in enumerate(trainloader):
            ...
            loss_H = args.hyper_parameters[0] * loss_mse + args.hyper_parameters[
                1] * (1 - loss_ssim) + args.hyper_parameters[2] * loss_adv + \
                     args.hyper_parameters[3] * loss_dnn
            loss_H.backward()
            optimizerH.step()
            ################DNNet#############
            optimizerN.zero_grad()
    ...
    def test(epoch):
        ...
        with torch.no_grad():
            for batch_idx, (input, label) in enumerate(testloader):
                ...
                loss_H = args.hyper_parameters[0] * loss_mse + \
                         args.hyper_parameters[1] * (1 - loss_ssim) + \
                         args.hyper_parameters[2] * loss_adv + \
                         args.hyper_parameters[3] * loss_dnn
                ...
                _, wm_predicted = dnn_outputs[args.batchsize:
                                              args.batchsize + args.wm_batchsize].max(
                    1)
                wm_correct += wm_predicted.eq(wm_label).sum().item()
                wm_total += args.wm_batchsize
        ...
        print('Real acc: %.3f  wm acc: %.3f wm cover acc: %.3f ' % (
            100. * real_correct / real_total, 100. * wm_correct / wm_total,
            100. * wm_cover_correct / wm_total))

        resultImg = torch.cat([wm_input, wm_img, secret_img], 0)
        torchvision.utils.save_image(resultImg,
                                     args.save_path + 'images/Epoch_' + str(
                                         epoch) + '_img.png',
                                     nrow=args.wm_batchsize,
    ...
    def save_loss_acc(epoch, loss, acc, train):
        ...
        ax1.set_xlabel(
            'Epoch(' + ",".join(str(l) for l in args.hyper_parameters) + ')')
        ax1.set_ylabel('Loss')
